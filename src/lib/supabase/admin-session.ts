...
import { accessTokenFromCookies } from "@/lib/jwt-exp";
import { readCachedAdmin, rememberAdmin, type AdminUser } from "@/lib/admin-auth-cache";
...
export const getAdminUser = cache(async function getAdminUser() {
  ...
  /* Skip the Auth round trip when this exact, still-unexpired token was
     verified moments ago. See lib/admin-auth-cache.ts for the reasoning and
     the bound. `undefined` means "no usable memory"; `null` is a remembered
     "not an admin", which is an answer. */
  const token = accessTokenFromCookies(cookieStore.getAll());
  const cached = readCachedAdmin(token);
