...
export async function rescheduleBooking(opts: {
  ...
  /* Exclude THIS booking from the availability check — it is being moved, so
     it must not be treated as occupying the calendar it is moving within. */
  const available = await isSlotAvailable(booking.category_id, opts.newDate, opts.newTime, opts.bookingId);
