...
const STEPS = [
  ...
  },
  {
    ...
    /* This lede and the last bullet below are overridden per care mode at
       render — see CHAPTER_THREE. The values kept here are the wording used
       when both switches are on, so the data still reads correctly alone. */
...
/**
 * Index of the antenatal chapter, whose lede and last bullet describe HOW you
 * are seen between appointments and therefore depend on the care mode. Held as
 * a constant so the substitution below cannot silently attach to the wrong
 * chapter if the order ever changes.
 */
const CHAPTER_THREE = 2;
const CHAPTER_THREE_ITEM = 3;
            ...
            {STEPS.map((raw, i) => {
              const s =
                i === CHAPTER_THREE
                  ? {
                      ...raw,
                      title: copy.chapterThreeTitle,
                      lede: copy.chapterThreeLede,
                      items: raw.items.map((t, n) => (n === CHAPTER_THREE_ITEM ? copy.chapterThreeItem : t)),
                    }
                  : raw;
              return (
