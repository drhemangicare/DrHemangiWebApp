...
export function BookingWizard({ clinicVisits = true }: { clinicVisits?: boolean }) {
  ...
  async function next() {
    ...
    if (step === 1) {
      ...
      /* Move to the calendar FIRST, then fetch.
         This used to `await loadAvailability(svc)` before `setStep(2)`, so
         pressing Next left you looking at the service list — the button did
         nothing visible for as long as the request took. The skeleton that
         was supposed to cover the wait lives on step 2, which you could not
         reach until the wait was already over. Advancing first means the
         loading state is actually seen, and the perceived delay drops to
         zero even before the query itself got faster. */
      setStep(2);
      void loadAvailability(svc);
      return;
    }
    if (step === 2) {
          ...
          <div className="bk-card">
            ...
            {step === 2 && (
              <div className="stepv on">
                ...
                <div className="date-rail">
                  {/* Date pills shaped like the real ones, so the rail keeps
                      its height and nothing jumps when the days arrive. A
                      one-line "Loading…" collapsed the rail to a few pixels
                      and then shoved the whole step down. */}
                  {loadingSlots &&
                    Array.from({ length: 7 }).map((_, i) => (
                      <span key={i} className="dte sk-dte" aria-hidden />
                    ))}
                  {loadingSlots && <span className="sr-only">Loading availability…</span>}
                ...
                {/* Skeleton times too, so the whole step has its final shape
                    while the request is in flight. */}
                {loadingSlots && (
                  <div className="slot-group">
                    <div className="slot-lbl"><span className="sk-line" style={{ width: 78 }} /></div>
                    <div className="slots">
                      {Array.from({ length: 10 }).map((_, i) => (
                        <span key={i} className="slot sk-slot" aria-hidden />
                      ))}
                    </div>
                  </div>
                )}

                {!loadingSlots && date && !day?.slots.length && (
            ...
            {step < 4 && (
              <div className="bk-nav">
                ...
                {/* Disabled while a request is in flight. Without this the
                    button stays live under the processing overlay, and a
                    double-tap on a slow connection fires a second booking. */}
                <button className="btn btn-p" onClick={next} disabled={processing !== null}>
                  {processing ? "Working…" : "Continue"}
                  {!processing && <svg><use href="#i-arr" /></svg>}
                </button>
              </div>
            )}
          </div>
