...
export default function AdminDiscountsPage() {
  ...
  /* Two requests, both through the shared cache — and the categories list is
     the SAME cache entry the pricing tab fills, so switching between the two
     tabs no longer re-fetches it. */
  const d = useAdminData<{ discounts: Discount[] }>("/api/admin/discounts");
  const c = useAdminData<{ categories: Category[] }>("/api/admin/categories");
  const discounts = d.data?.discounts ?? [];
  const categories = c.data?.categories ?? [];
  const loading = d.loading || c.loading;
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="space-y-3">
        {discounts.map((d) => (
          <div key={d.id} className="a-card rounded-2xl p-4 sm:p-5 flex flex-wrap items-center gap-x-4 gap-y-3 shadow-sm">
