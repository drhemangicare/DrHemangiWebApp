...
export default function AdminAvailabilityPage() {
  ...
  /* Through the shared cache, so the schedule is on screen the moment you
     open the tab and only the refresh happens in the background. */
  const wh = useAdminData<{ working_hours: WorkingHour[] }>("/api/admin/availability/working-hours");
  const bs = useAdminData<{ blocked_slots: BlockedSlot[] }>("/api/admin/availability/blocked-slots");
  /* Memoised, not `?? []` inline: an empty-array literal is a new identity on
     every render, which would make the grouping useMemo below recompute every
     time and defeat the point of having it. */
  const hours = useMemo(() => wh.data?.working_hours ?? [], [wh.data]);
  const blocked = useMemo(() => bs.data?.blocked_slots ?? [], [bs.data]);

  const load = useCallback(() => {
    invalidateAdminCache("/api/admin/availability");
    wh.refresh();
    bs.refresh();
  }, [wh, bs]);
  ...
  return (
    <div className="max-w-3xl space-y-6 sm:space-y-8">
      ...
      {/* ── Weekly hours ── */}
      <section className="a-card rounded-2xl p-4 sm:p-5 shadow-sm">
